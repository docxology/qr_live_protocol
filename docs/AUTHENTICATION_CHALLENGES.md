...
## Forward-Compatible QR Payloads (v1.1.0+)

QRLP's `QRData.from_json()` method silently ignores unknown fields when parsing
QR payloads. This means:

- **Older verifiers** can scan QR codes generated by newer QRLP versions that
  include additional fields not yet known to the older verifier.
- **Newer verifiers** can still scan QR codes from older versions — missing fields
  default to `None` and are handled gracefully.
- **No breaking changes** are required when adding new fields to the QR payload
  format — the `from_json` method filters to known dataclass fields only.

This forward-compatibility design ensures that QR codes generated today remain
verifiable by future QRLP versions, and vice versa.
