...
class TestQRLiveWebServer:
    ...
    def test_api_verify_rejects_empty_qr_payload(self, web_settings):
        """Test /api/verify uses real verifier semantics, not JSON-only validation."""
        server = QRLiveWebServer(web_settings)

        with server.app.test_client() as client:
            response = client.post('/api/verify', json={"qr_data": "{}"})
            assert response.status_code == 200
            data = json.loads(response.data)
            assert data["valid"] is False
            assert data["valid_json"] is False

    def test_api_verify_accepts_current_local_qr(self, web_settings, tmp_path):
        """Test /api/verify accepts a QR generated by its configured verifier."""
        config = QRLPConfig()
        ...
        qrlp = QRLiveProtocol(config, key_manager=KeyManager(str(tmp_path / "keys")))
        qr_data, _ = qrlp.generate_single_qr({"web": "verify"}, sign_data=True)
        server = QRLiveWebServer(web_settings, verifier=qrlp)

        with server.app.test_client() as client:
            response = client.post('/api/verify', json={"qr_data": qr_data.to_json()})
            assert response.status_code == 200
            data = json.loads(response.data)
